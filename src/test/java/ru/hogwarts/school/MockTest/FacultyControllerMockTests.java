package ru.hogwarts.school.MockTest;
...
public class FacultyControllerMockTests {
    ...
    public void testGetFacultiesByColor() throws Exception {
        ...
        when(facultyRepository.getByColor("green")).thenReturn(faculties);

        mockMvc.perform(MockMvcRequestBuilders
                        .get("/faculty//faculty-by-color/green")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
    ...
    public void testGetStudentsByColorOrName() throws Exception {
        ...
        when(facultyRepository.findByColorOrNameIgnoreCase("Gryffindor", "green")).thenReturn(faculties);

        mockMvc.perform(MockMvcRequestBuilders
                        .get("/faculty//filter?color=green&name=Gryffindor")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$.size()").value(faculties.size()))
